...
import { getStorage, ref as storageRef, uploadString, getBlob, deleteObject } from "firebase/storage";
...
// ── Firebase Storage: file binari (immagini referenze, loghi clienti) ──
...
const fileStore = getStorage(firebaseApp);

// carica un file (data URL) su Storage e restituisce il path salvato
const caricaFileSuStorage = async (path, dataUrl) => {
  const fullPath = `${ROOT}/${path}`;
  await uploadString(storageRef(fileStore, fullPath), dataUrl, "data_url");
  return fullPath;
};

// scarica un file da Storage in modo autenticato (via SDK, non un link pubblico)
// e restituisce un blob URL locale pronto per un <img src=...>
const scaricaFileDaStorageComeBlobUrl = async path => {
  const blob = await getBlob(storageRef(fileStore, path));
  return URL.createObjectURL(blob);
};
const eliminaFileSuStorage = async path => {
  try {
    await deleteObject(storageRef(fileStore, path));
  } catch (e) {/* già assente: ok */}
};
...
function RepositoryReferenzeInterna({
  ...
  // ── immagini per referenza: file su Storage, metadati leggeri (path + didascalia) su RTDB ──
  const caricaImmagini = async refId => {
    try {
      const r = await storage.get(imgKey(refId));
      const meta = r && r.value ? JSON.parse(r.value) : [];
      const conBlob = await Promise.all(meta.map(async im => {
        try {
          const dataUrl = await scaricaFileDaStorageComeBlobUrl(im.path);
          return {
            ...im,
            dataUrl
          };
          ...
          return {
            ...im,
            dataUrl: null
          }; // file mancante/non accessibile: mostrato vuoto invece di rompere la UI
        }
      }));
      return conBlob;
    } catch (e) {
  ...
  const salvaImmagini = async (refId, arr) => {
    try {
      // recupera i metadati precedenti per capire quali immagini sono state rimosse (da eliminare da Storage)
      let precedenti = [];
      try {
        const r = await storage.get(imgKey(refId));
        precedenti = r && r.value ? JSON.parse(r.value) : [];
      } catch (e) {}
      const idAttuali = new Set(arr.map(im => im.id));
      await Promise.all(precedenti.filter(p => !idAttuali.has(p.id)).map(p => eliminaFileSuStorage(p.path)));

      // carica su Storage solo le immagini nuove (prive di "path"); quelle già salvate restano tali
      const metaFinale = await Promise.all(arr.map(async im => {
        if (im.path) return {
          id: im.id,
          path: im.path,
          didascalia: im.didascalia || ""
        };
        const path = await caricaFileSuStorage(`immagini/${refId}/${im.id}.jpg`, im.dataUrl);
        return {
          id: im.id,
          path,
          didascalia: im.didascalia || ""
      ...
      await storage.set(imgKey(refId), JSON.stringify(metaFinale));
    } catch (e) {
      showToast("Salvataggio immagini non riuscito");
    }
  };

  // ── logo cliente: file su Storage, path salvato su RTDB ──
  const caricaLogo = async clientId => {
    try {
      const r = await storage.get(logoKey(clientId));
      const path = r && r.value ? r.value : null;
      if (!path) return null;
      return await scaricaFileDaStorageComeBlobUrl(path);
    } catch (e) {
      return null;
    }
  };
  const salvaLogo = async (clientId, dataUrl) => {
    try {
      // elimina l'eventuale logo precedente su Storage prima di sostituirlo
      try {
        const r = await storage.get(logoKey(clientId));
        if (r && r.value) await eliminaFileSuStorage(r.value);
      } catch (e) {}
      if (dataUrl) {
        const path = await caricaFileSuStorage(`loghi/${clientId}.png`, dataUrl);
        await storage.set(logoKey(clientId), path);
      } else {
        await storage.delete(logoKey(clientId));
      }
    } catch (e) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // caricati su Storage, in RTDB si salva solo il path
      for (const [clientId, dataUrl] of Object.entries(LOGHI_SEED)) {
        try {
          const esiste = await storage.get(logoKey(clientId));
          if (esiste && esiste.value) continue; // già presente, non sovrascrivere
        } catch (e) {/* non presente: procedi con il seed */}
        try {
          const path = await caricaFileSuStorage(`loghi/${clientId}.png`, dataUrl);
          await storage.set(logoKey(clientId), path);
        } catch (e) {/* upload seed non riuscito: non bloccare il caricamento dell'app */}
      }
      setRefs(refsData);
